...
static char rcsid[] __attribute__((unused)) = "$Header: /usr/local/cvsroot/dartmouth/blitzserv/notify/notifyd.c,v 3.15 2002/11/01 20:04:19 davidg Exp $";
...
boolean_t remove_entry(long uid, atpaddr *regaddr, char *reason);
...
void initialize () {
    ...
    if (!m_noappletalk && !ddpinit()) {	/* set up ddp package (after config) */
        t_errprint("DDP initialization failed; disabling AppleTalk");
        m_noappletalk = TRUE;
    }
...
/* do_notify --
    ...
    Internal routine to send notification.  
    Called from the TCP interface, and by the server itself
    (to send sticky notifications.)
...
int do_notify(long uid, long typ, long id, notifydat data, long len,
	...
	for (i = 0; i < bp->count; i++) {
		...
		if (reset) {		/* make list of entries for this uid */
			...
			remtab = t_reallocf(remtab, remmax * sizeof(atpaddr));
		    }
	...
	for (i = 0; i < remcount; ++i) { /* get rid of all the entries we saw */
	    remove_entry(uid, &remtab[i], "reset");
	}
	t_free(remtab);
    }
    pthread_mutex_unlock(&notifytab_lock);
...
void notify_one(long typ, long id, notifydat data, long len, notifybuck *bp, int i) {
    ...
    reqp->pkt.ddptype = DDP_ATP; 	/* set the DDP type */
...
void notify_tcp(any_t state_) {
	...
	t_fseek(&state->conn, 0, SEEK_CUR); /* set up to read next command */
...
void nbp_register(char *object, char *type, ddpsockp ddpsock) {
    ...
    np->regpkt.ddptype = DDP_NBP_REG;	/* set ddp type */

    reghdr = (nbphdr *) np->regpkt.ddpdata;
    reghdr->typecount = NBP_TYPECOUNT(NBP_REG, 1); /* register; one name per packet */
...
void tres (atphdr *atp, atpaddr *clientaddr) {
	...
	if (sta == NC_NOUSER) {		/* if notification rejected */
	    ...
	    remove_entry(p->uid, &p->clientaddr, "client rejects that uid");
	    pthread_mutex_unlock(&notifytab_lock);	
	}
...
sta_typ reg_request(req	*reqp, atphdr *atp) {
    ...
    */
	...
	if (regaddr.port == 0) {	/* 0: use same port request came from */
	    regaddr.port = reqp->clientaddr.port;
	} else {
...
boolean_t remove_entry(long uid, atpaddr *regaddr, char *reason) {
    ...
    at_regaddr = (ataddr *) &regaddr->addr;
    if (regaddr->family == AF_APPLETALK) {
	t_sprintf(buf, "Remove entry %ld/%ld/%ld for uid %ld: %s\n",
		    (long) at_regaddr->at_net,
		    (long) at_regaddr->at_node,
		    (long) at_regaddr->at_sock,
		    uid, reason);
	...
	pthread_mutex_lock(&inet_ntoa_lock);
	t_sprintf(buf, "Remove entry %s %ld for uid %ld: %s\n",
		inet_ntoa(*((struct in_addr *) &regaddr->addr)),
		(long) ntohs(regaddr->port),
		uid, reason);
	pthread_mutex_unlock(&inet_ntoa_lock);
...
void atp_periodic(any_t zot) {
	...
	for (reqp = rtxreq.head; reqp; reqp = next) {
		...
		if (gonecount == gonemax) {
		    ...
		    gonelist = t_reallocf(gonelist, gonemax * sizeof(struct gone));
		}
...
int atpread(atpskt *skt, ddpbuf *bufp, atpaddr *source) {
    ...
    switch(source->family) {
		...
		if (cc < ATPHDRLEN + 1) {
		    if (cc < 0) {	/* error */
			/* ignore errors generated by sendto() */
			if (pthread_errno() != ECONNREFUSED &&
			    pthread_errno() != EHOSTUNREACH) {
				t_perror("atpread: recvfrom");
			}
		    } else {
...
int atpwrite(atpskt *skt, ddpbuf *bufp, int len, atpaddr *remoteaddr) {
    ...
    switch(remoteaddr->family) {
    	case AF_INET:
	    bufp->ddptype = DDP_ATP;	/* set DDP type */
	    /* set up UDP address */
		...
		char buf[256];
        	...
        	pthread_mutex_lock(&inet_ntoa_lock);
        	t_sprintf(buf, "atpwrite: sendto %s %ld\n",
                	inet_ntoa(*((struct in_addr *) &remoteaddr->addr)),
                	(long) ntohs(remoteaddr->port));
        	pthread_mutex_unlock(&inet_ntoa_lock);
		t_perror(buf);
	    }
	    return s;
